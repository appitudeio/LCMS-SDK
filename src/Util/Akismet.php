...
class Akismet
{
     ...
     * @throws \Exception
     */
		function __construct(string $_api_key)
		{
		...
		public function validate(Request $request): Bool
		{
	        ...
	        if(!$this->isValidAPIKey($this->comment['blog'])) 
	        {
	            throw new Exception("The API key passed in Akismet constructor is invalid. Please obtain a valid one from https://akismet.com");
	        }

			$this->setCommentAuthorEmail($request->get("email"));

			if($request->get("name"))
			{
				$this->setCommentAuthor($request->get("name"));
			}

			if($this->isCommentSpam())
			{
				throw new Exception("Is spam email: " . $request->get("email"));
			}
     ...
     * @return bool
     */
    private function isValidAPIKey($website) : bool
    {
